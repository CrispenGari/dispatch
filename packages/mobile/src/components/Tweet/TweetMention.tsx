...
const TweetMention: React.FunctionComponent<Props> = ({
  mention: { id },
  navigation,
  from,
}) => {
  ...
  const {
    ...
    data: mention,
    refetch,
  } = trpc.mention.mention.useQuery({
    id,
  });

  trpc.tweet.onTweetUpdate.useSubscription(
    { uid: me?.id || "", tweetId: mention?.tweetId || "" },
    {
      onData: async (data) => {
        if (!!data) {
          await refetch();
  ...
  const viewProfile = () => {
    ...
    if (!!!mention?.tweet) return;
    navigation.navigate("User", { from: "Feed", id: mention.tweet.userId });
  };
  ...
  const reactToTweet = async () => {
    ...
    if (!!!mention?.tweet) return;
    mutateReactToTweet({ id: mention.tweet.id }).then(async (_res) => {});
  };
  ...
  const view = () => {
    ...
    if (!!!mention?.tweet) return;
    navigation.navigate("Tweet", { id: mention.tweet.id, from });
  };
  ...
  if (!!!mention?.tweet || fetching) return <TweetSkeleton />;
  const { tweet } = mention;
  return (
    <TouchableOpacity
      onPress={view}
